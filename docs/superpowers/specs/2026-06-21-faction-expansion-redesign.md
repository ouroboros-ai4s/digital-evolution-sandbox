...
| # | 偏差 | 代码位置 | 后果 |
|---|---|---|---|
| **B1 播种** | BB0 铺满全部 16384 格,世界一开始就满 | `world.py:29` `strain_id[:,:,0]=bb0` | 无扩张过程、无相遇前沿、空间均质 CV≈0 |
| **B2 免疫** | 对抗免疫按**序列**判(`sid_i!=sid_j`),非按阵营 | `antagonism.py:61` | 同阵营突变后代互斗(内战);异阵营收敛到同序列反而免疫 |
| **B3 扩张** | BB0 自带繁衍基元 `F4Nr1` 只向北一个方向 | `registry.py:23` `((-1,0),)` | 即使改 4 格起步,也只长成竖条、永不相遇 |
...
- 改为 `F4Nr4`(四方向 `((-1,0),(1,0),(0,-1),(0,1))`,f=0.50)——`registry.py:229` 已定义为"标准扩张基准(=原F4)"
- 改动点:`BB0_TEMPLATE["layout"]` 里把 F 位的字母从 F4Nr1 换成 F4Nr4
- 影响:扩张波从 1D 竖条变 2D 钻石波,~1 格/tick/轴,四象限中心(等距 64)→ 前沿约 **tick 32 相遇**,世界约 **tick 60-65 填满**,T=200 余量充足
- **私货检查:** 四阵营全用同一条改后 BB0,无阵营拿不同基元 → 对称,不偏袒。F4Nr4 是设计文档已锁的中性扩张基准,非为某方加强。
...
2. **扩张发生:** 占用格数从 4 单调增长,~tick 60-65 接近填满(非 t=0 即满)
3. **相遇对抗:** 存在某 tick 起跨 faction 同格 → 对抗触发 → 减员事件可观测
