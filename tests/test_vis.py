...
def test_relabel_invariance_vis_path_does_not_read_fzp_magnitude(monkeypatch):
    """Spec S1 section 6: shuffling _F/_Z/_P magnitudes must not affect the
    vis path. phenotype().vis_sum, .n_count, .vis_mode read only VIS and
    ALPHABET — never _F/_Z/_P magnitudes."""
    seq = registry.BB0_TEMPLATE["layout"]

    # Compute vis aggregates before any perturbation.
    ...
    monkeypatch.setitem(registry._Z, "BroadSweep",
                        (0.99, (("F",), ("Z",)), 99, 0))
    # Shuffle _P magnitudes.
    monkeypatch.setitem(registry._P, "P_hotspot", (0.0, 99))
    monkeypatch.setitem(registry._P, "P_base", (0.05, 1))

    # Compute vis aggregates after perturbation — must be identical.
    p_after = phenotype(seq)

    assert p_after.vis_sum  == p_before.vis_sum,  \
        f"vis_sum leaked f/z/p: {p_before.vis_sum} -> {p_after.vis_sum}"
    assert p_after.n_count  == p_before.n_count,  \
        f"n_count leaked f/z/p: {p_before.n_count} -> {p_after.n_count}"
    assert p_after.vis_mode == p_before.vis_mode, \
        f"vis_mode leaked f/z/p: {p_before.vis_mode} -> {p_after.vis_mode}"
